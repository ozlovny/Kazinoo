...
def load_data():
    ...
    if 'currency' not in BOT_DATA:
        BOT_DATA['currency'] = {
            'name': 'MH',          # Назва валюти
            'rate_to_usdt': 0.01   # 1 unit = USDT
        }
    if 'limits' not in BOT_DATA:
        BOT_DATA['limits'] = {
            'min_deposit': 1.0,   # минимальное пополнение (в USDT)
            'min_withdraw': 10.0, # минимальный вывод (в валюте)
            'min_bet': 1.0        # минимальная ставка (в валюте)
        }
    if 'game_coeffs' not in BOT_DATA:
        BOT_DATA['game_coeffs'] = {
            'cube': 6.0,                # точное число 1-6
            'duel': 2.0,                # дуэль 50/50
            'even': 2.0,                # чёт
            'odd': 2.0,                 # нечет
            'more': 2.0,                # больше (>3)
            'less': 2.0,                # меньше (<4)
            'red': 2.0,                 # дартс красный
            'white': 2.0,               # дартс белый
            'darts_center': 5.0,        # дартс центр
            'basket_score': 2.0,        # баскет корзина
            'basket_miss': 2.0,         # баскет мимо
            'football_goal': 2.0,       # футбол гол
            'football_miss': 2.0,       # футбол мимо
            'bowling': 5.0,             # боулинг страйк
            'slots': 10.0,               # слоты 777/тройка
            'pvp_darts': 2.0,
            'pvp_basket': 2.0,
            'pvp_football': 2.0,
            'pvp_bowling': 2.0
...
def main_menu_markup(user_id	):
    ...
    markup.add('🎲 Играть', '🎁 Бонус')
    markup.add('👤 Партнёры', '🖥️ Профиль')
...
# --- Обработка активации чека ---
    ...
    bot.send_message(user_id, 
                     f"<pre>👋 Добро пожаловать! </pre>\n"
                     f"<b>└🚀 Бот создан с помощью: @TGquyfaCreatorBot</b>", # Используем HTML
                     parse_mode='HTML', # Изменено на HTML
...
# 🚀 Создано в отдельной строке
    markup.row(types.InlineKeyboardButton(text="🚀 Создано в", url="https://t.me/TGquyfaCreatorBot"))
